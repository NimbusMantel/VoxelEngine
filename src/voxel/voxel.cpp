...
namespace manVox {
	void init() {
		uint32_t r[32] = { 0x80000000, 0x0, 0x0, 0x0, 0x10000000, 0x0, 0x0, 0x0, 0x20000000, 0x0, 0x0, 0x0, 0x30000000, 0x0, 0x0, 0x0, 0x40000000, 0x0, 0x0, 0x0, 0x50000000, 0x0, 0x0, 0x0, 0x60000000, 0x0, 0x0, 0x0, 0x70000000, 0x0, 0x0, 0x0 };
		
		std::unique_ptr<uint32_t[]> root(new uint32_t[32]());

		memcpy(root.get(), &r, 32 * 4);
		
		cPriorityQueue[(cPriorityIndex - 1) & cPriorityMax].push_back(std::make_unique<INS_CTG_RLD>(INS_CTG_RLD(0, 0, std::move(root))));
	}
}